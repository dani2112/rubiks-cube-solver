...
public class CubeEdgeDetector {
	...
	public static void detect(Mat image) {
		...
		/* Calculate abs of the gradient image 
		 * absdiff takes absolute difference between first and second parameter
		 * as second parameter is all zero this will be equivalent to abs(gradientX)*/
		...
		SlidingWindowExecutor.process(integralImage, rectangleRows, rectangleCols, rowStep, colStep, new SlidingWindowAlgorithm() {

			@Override
			public void processRoi(int row, int col) {
				double areaValue = getAreaValue(row, col, rectangleRows, rectangleCols, integralImage.cols(), integralValues);
				if(areaValue > 10000) {
					Imgproc.drawMarker(image, new Point(col, row), new Scalar(255,0,0), Imgproc.MARKER_CROSS, 5, 2, Imgproc.LINE_4);
				}
			}
	...
	private static double getAreaValue(int upperLeftRow, int upperLeftCol, int rectangleRows, int rectangleCols, int cols, double[] integralValues) {
